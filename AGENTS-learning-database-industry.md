...
每个专题选若干代表系统。重点系统按完整模板拆解，辅助系统用于横向对照。
...
开源系统涉及实现细节时，优先 clone 到本地并用源码验证。闭源系统不强行源码验证，但必须标注公开资料来源；无法直接验证的判断要写明“基于公开资料推断”。
...
| 阶段 | 专题 | 重点系统 |
| --- | --- | --- |
| 1 | 现代数据库行业全景 | PostgreSQL、MySQL、RocksDB、Redis、Lucene、Snowflake、BigQuery、Spanner、TiDB、ClickHouse、Doris、Milvus |
...
5. 根据用户本次意图决定是答疑、更新索引、创建新报告，还是推进下一专题。
