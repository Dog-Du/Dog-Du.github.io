...
开源系统必须 clone 到本地源码目录，默认放在 `D:\program\<repo-name>`，并用源码验证关键判断；如果本地已经存在对应仓库，则复用并记录实际路径。若因为网络、权限或仓库不可用导致暂时无法 clone，必须记录阻塞原因，并且不能写源码级结论。闭源系统不强行源码验证，但必须标注公开资料来源；无法直接验证的判断要写明“基于公开资料推断”。
