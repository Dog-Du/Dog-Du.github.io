...
当前推进规则以 `AGENTS-learning-database-industry.md` 和 `content/posts/learning-database-industry-day000-index.md` 为准。

最终决策：

- `Topic` 是学习阶段和问题域，不等于一篇文章。
- `Day` 是单篇文章编号单位。
- `Topic 1：现代数据库行业全景` 不按数据库系统逐个展开，而是按后续专题做预览文章。
- `Topic 2+` 才按数据库系统展开，每个重点系统各一篇，并配套专题开篇和专题收束。
- 每个专题都需要形成闭环：开篇、主体文章、收束。
...
专题用于组织进度，Day 用于标识单篇文章。

`Topic 1` 是行业地图和后续专题预览，不逐个数据库系统展开。它重点回答后续每个问题域为什么独立存在、解决什么需求、有哪些代表系统、后续应该怎么学。

`Topic 2+` 才进入系统级文章。每个专题选择若干代表系统，每个重点系统按固定模板拆解，重点回答它为什么出现、如何解决问题、把复杂性转移到哪里、哪些问题只能靠插件或外围系统变相解决、在哪些场景下会失败。
...
开源系统必须 clone 到本地源码目录，默认放在 `local-repos/<repo-name>`，并用源码验证关键判断；如果本地已经存在对应仓库，则复用并记录实际路径。若因为网络、权限或仓库不可用导致暂时无法 clone，必须记录阻塞原因，并且不能写源码级结论。闭源系统不强行源码验证，但必须标注公开资料来源；无法直接验证的判断要写明“基于公开资料推断”。

## 三个月专题序列

| 周期 | 专题 | 重点系统 | 学习目标 |
| --- | --- | --- | --- |
| 第 1-2 周 | 现代数据库行业全景 | 后续专题预览：传统 OLTP、LSM、分布式 SQL、云原生存算分离、OLAP、搜索向量、Lakehouse | 建立行业地图，理解后续每个问题域为什么独立存在 |
...
每个专题产出一组调研报告式个人学习文档。它偏内部学习，不追求公开博客叙事，重点是结构、对比、结论、缺陷、badcase、引用和工程启发。
...
第一篇建议从“现代数据库行业全景：传统系统如何演进出现代数据库”开始。它不深挖源码，重点建立行业地图、系统分类、演进关系和后续专题入口。

第二篇不是进入 PostgreSQL 系统深挖，而是写 `传统 OLTP 与存储基础预览`。完成 Topic 1 的各专题预览和收束后，再从 Topic 2 开始逐步把 PostgreSQL、MySQL/InnoDB、SQLite 等系统纳入路径级拆解。
