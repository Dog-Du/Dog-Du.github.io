...
- Open-source systems must be cloned to a local source directory and include official docs or design docs plus local source anchors when the report makes implementation-level claims.
...
For open-source systems, clone or reuse local source code before making implementation-level claims. The default local path is `local-repos/<repo-name>`. If cloning is blocked by network, permissions, or repository availability, record the blocker and do not write source-level conclusions. Do not treat third-party summaries as final conclusions when local source or official docs can verify the point.
...
When the user asks to learn, discuss, review, compare, or persist modern database industry study content in this repository, follow [AGENTS-learning-database-industry.md](something-prompt/learn-database-industry/AGENTS-learning-database-industry.md) as the task-specific rule set.
...
- For open-source systems, clone the project to local source code first, then verify implementation-level claims against that local source code when the topic requires that depth.
...
- 开源系统：必须 clone 到本地源码目录，默认路径为 `local-repos/<repo-name>`；如果本地已存在则复用，并在报告中记录实际源码路径、关键目录、关键类或函数。
...
记录所有参考来源。开源系统必须先 clone 到本地源码目录，默认路径为 `local-repos/<repo-name>`；如果本地已存在则复用，并记录实际源码路径、关键目录、关键文件、类或函数。闭源系统记录官方公开资料来源。如果开源系统暂时无法 clone，必须记录阻塞原因，并且不能写源码级结论。
```
