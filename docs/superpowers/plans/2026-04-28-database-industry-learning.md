...
For open-source systems, clone or reuse local source code before making implementation-level claims. The default local path is `D:\program\<repo-name>`. If cloning is blocked by network, permissions, or repository availability, record the blocker and do not write source-level conclusions. Do not treat third-party summaries as final conclusions when local source or official docs can verify the point.
...
记录所有参考来源。开源系统必须先 clone 到本地源码目录，默认路径为 `D:\program\<repo-name>`；如果本地已存在则复用，并记录实际源码路径、关键目录、关键文件、类或函数。闭源系统记录官方公开资料来源。如果开源系统暂时无法 clone，必须记录阻塞原因，并且不能写源码级结论。
```
