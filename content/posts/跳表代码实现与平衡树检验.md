...
- 算法与数据结构
title: 跳表代码实现与平衡树检验
slug: algo-skiplist-full-impl
summary: 基于带注释的完整代码介绍跳表的层高生成、跨度维护与有序集合操作，并用洛谷 P3369 作为正确性检验。
commentTerm: "跳表代码实现与平衡树检验 | DogDu's blog"
featureimage: "images/covers/cover_32_snow.webp"
---
<!--more-->
跳表经常被拿来和平衡树对比：它同样支持有序集合的常见操作，但实现思路更偏向“多层索引链表”，结构上直观，写起来也相对轻巧。

这篇文章直接给出一份带完整注释的实现，重点放在三个位置：随机层高生成、`span` 维护，以及如何用这些信息完成 `get_rank` / `get_val` 这类顺序统计操作。文末也用洛谷 P3369 作为正确性检验。

代码里使用的是 C++ 的 `<random>` 库，而不是传统的 `rand()` / `srand()`。如果准备自己动手改模板，先熟悉一下这部分会更顺手。
