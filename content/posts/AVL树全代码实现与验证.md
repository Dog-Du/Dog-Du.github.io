...
<!--more-->
建议看这篇文章之前，先去学习AVL树定义和一些必要知识。

收获：

1.吃屎，然后知所以然

2.say is easy,show me the code.

3.制定规则，然后遵守规则

诚挚推荐B站up AgOH大佬 orz ：【AgOHの数据结构】平衡树专题之叁 树旋转与AVL树_哔哩哔哩_bilibiligg

感谢大佬二十多分钟拯救我的睡眠情况。实际上这里的代码只是把大佬的代码改成指针的形式随便写成一个类而已。

我本来的写法是和书本（数据结构-严蔚敏）一致的，而且使用的非递归算法，结构体中带的是bf和parent，结果连着好几天晚上看代码看半天，一坐就是几个小时说不痛苦都是假的。实现难度是真的大。但是看了大佬视频之后，头不疼了，眼不花了，二百行就搓出来了。

首先一定需要明确的是，AVL树的旋转有且仅有四种，LLR，LRLR，RRL和RLRL型。不可能再有别的。我在看别的人的博客的时候，发现有人在删除函数中荒谬的创造出了第五种甚至第六种旋转类型。
