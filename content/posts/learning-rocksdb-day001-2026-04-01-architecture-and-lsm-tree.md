...
lastmod: 2026-04-20T16:45:21+08:00
...
featureimage: "images/covers/rocksdb/day001-architecture-4ve76m.webp"
...
- `MemTable`
  - 某个 `CF` 当前的内存写缓冲
  - 不是整个 DB 只有一个，而是**每个 CF 一套**
- `WAL`
  - 整个 `DB` 共用的预写日志
  - 不是每个 `CF` 一份
- `SuperVersion`
  - 某个 `CF` 当前稳定可读视图
  - 里面打包了这个 `CF` 的 `mem + imm + current version`
- `VersionSet`
  - 整个 `DB` 的版本元数据管理器
...
- `DB` 管全局
- `CF` 管逻辑隔离
- `MemTable` 和 `SuperVersion` 是 **CF 维度**
- `WAL`、`VersionSet`、`MANIFEST` 是 **DB 维度**

先看关系图：

```mermaid
flowchart TD
    DB["DB / DBImpl<br/>整个数据库实例"] --> CFA["CF A"]
    DB --> CFB["CF B"]
    DB --> WAL["WAL<br/>整个 DB 共用"]
    DB --> VS["VersionSet<br/>整个 DB 共用"]
    VS --> MANI["MANIFEST<br/>整个 DB 共用"]
...
- 外部参考图：
  - [rocksdb_lsm_flow.svg](https://xiaodongq.github.io/images/rocksdb_lsm_flow.svg)

![RocksDB LSM 流程图](https://xiaodongq.github.io/images/rocksdb_lsm_flow.svg)
