...
- 当前最近一次主题：`Day 007：搜索、向量与生态补丁预览`
- 当前下一步：`Day 008：Lakehouse 与对象存储表格式预览`
...
| 8 | 预览 | Lakehouse 与对象存储表格式预览 | `learning-database-industry-day008-YYYY-MM-DD-lakehouse-table-format-preview.md` | `next` |
...
| 008 | TBD | 现代数据库行业全景 | 专题预览 | `learning-database-industry-day008-YYYY-MM-DD-lakehouse-table-format-preview.md` | `next` |
...
下一篇建议创建：`learning-database-industry-day008-YYYY-MM-DD-lakehouse-table-format-preview.md`

主题：`现代数据库行业全景之 Lakehouse 与对象存储表格式预览`

建议重点：

- 说明为什么搜索、向量与生态补丁之后，需要单独看 Lakehouse、对象存储和表格式。
- 解释 Iceberg、Delta Lake、Paimon 后续为什么各自需要单独成文或横向对照。
- 建立 object file、manifest、metadata tree、snapshot、delete file、compaction、schema evolution 和多引擎共享的共同比较框架。
- 预先列出小文件、commit 冲突、旧 snapshot 保留、delete file 膨胀、catalog 可用性、多引擎一致性和成本等 badcase。
