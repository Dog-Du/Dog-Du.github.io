...
- 当前最近一次主题：`Day 011：PostgreSQL OLTP 存储系统文章`
- 当前下一步：`Day 012：MySQL/InnoDB OLTP 存储系统文章`
...
| 2 | 系统 | PostgreSQL | `learning-database-industry-day011-2026-06-10-postgresql-oltp-storage.md` | `done` |
| 3 | 系统 | MySQL/InnoDB | `learning-database-industry-day012-YYYY-MM-DD-mysql-innodb-oltp-storage.md` | `next` |
...
| 011 | 2026-06-10 | 传统 OLTP 与存储基础 | 系统文章 | `learning-database-industry-day011-2026-06-10-postgresql-oltp-storage.md` | `done` |
| 012 | TBD | 传统 OLTP 与存储基础 | 系统文章 | `learning-database-industry-day012-YYYY-MM-DD-mysql-innodb-oltp-storage.md` | `next` |
...
下一篇建议创建：`learning-database-industry-day012-YYYY-MM-DD-mysql-innodb-oltp-storage.md`

主题：`MySQL/InnoDB OLTP 存储系统文章`

建议重点：

- 从 InnoDB clustered index、secondary index 回表、redo、undo、binlog、buffer pool、doublewrite、purge 和 online DDL 展开。
- 同时读取 MySQL 8.4 官方文档和 `local-repos/mysql-server` 本地源码入口，不把文档级描述直接写成源码级结论。
- 优先验证写入路径、事务提交路径、purge 回收路径、二级索引维护路径和 binlog/redo/undo 的边界。
