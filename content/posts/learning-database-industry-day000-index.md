...
- 当前最近一次主题：`Day 001：现代数据库行业全景`
- 当前下一步：`Day 002：传统 OLTP 与存储基础`
...
| 002 | TBD | 传统 OLTP 与存储基础 | `learning-database-industry-day002-YYYY-MM-DD-traditional-oltp-storage-foundations.md` | `planned` |
...
下一篇建议创建：`learning-database-industry-day002-YYYY-MM-DD-traditional-oltp-storage-foundations.md`

主题：`传统 OLTP 与存储基础：PostgreSQL、MySQL/InnoDB、SQLite`

建议重点：

- page、heap、B+Tree、clustered index 的数据组织。
- WAL、redo、undo、binlog 的职责边界。
- MVCC、长事务、VACUUM/purge 的 badcase。
- buffer pool、OS page cache、二级索引与在线 DDL。
