...
- 当前最近一次主题：`Day 002：传统 OLTP 与存储基础预览`
- 当前下一步：`Day 003：LSM 与嵌入式存储引擎预览`
...
| 3 | 预览 | LSM 与嵌入式存储引擎预览 | `learning-database-industry-day003-YYYY-MM-DD-lsm-embedded-storage-preview.md` | `next` |
...
| 003 | TBD | 现代数据库行业全景 | 专题预览 | `learning-database-industry-day003-YYYY-MM-DD-lsm-embedded-storage-preview.md` | `next` |
...
下一篇建议创建：`learning-database-industry-day003-YYYY-MM-DD-lsm-embedded-storage-preview.md`

主题：`现代数据库行业全景之 LSM 与嵌入式存储引擎预览`

建议重点：

- 说明为什么传统 OLTP 的 B+Tree/page update 之外还需要 LSM。
- 解释 RocksDB、BadgerDB、Pebble 后续为什么各自需要单独成文。
- 建立 WAL、memtable、SST、value log、compaction、snapshot、iterator、GC 的共同比较框架。
- 预先列出写放大、读放大、空间放大、write stall 和 compaction 资源竞争等 badcase。
