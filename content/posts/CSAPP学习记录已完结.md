...
```
for (i = 0; i < N; i += 16) {           for (j = 0; j < M; j += 16) {               for (k = i; k < i + 16 && k < N; ++k) {                   for (l = j; l < j + 16 && l < M; ++l) {                       B[l][k] = A[k][l];                   }               }           }       }
```
...
```
char naive_rotate_descr[] = "naive_rotate: Naive baseline implementation"; void naive_rotate(int dim, pixel *src, pixel *dst) {     int i, j;      for (i = 0; i < dim; i++) 	for (j = 0; j < dim; j++) 	    dst[RIDX(dim-1-j, i, dim)] = src[RIDX(i, j, dim)]; }
```
...
```
static void shark_heap() {   if (!IS_FRONT_FREE(mm_brk)) {     return;   }    void *ptr = GET_FRONT_PTR(mm_brk);   REMOVE_FROM_LIST(ptr);    int shark = GET_SIZE(ptr) + HEAD_SIZE;   mm_brk -= shark;   mem_reset_brk();   mem_sbrk(mm_brk - mem_heap_lo() + HEAD_SIZE);    SET_SIZE(ptr, 0);   SET_FREEBIT(ptr, 0);   mm_brk = ptr; }
```
