...
1. PostgreSQL 的专有名词先说清楚：relation、fork、page、tuple、TID、xmin/xmax、snapshot、WAL、LSN、VM、FSM["free space map"]、HOT、VACUUM、catalog、extension。
...
| page / block | PostgreSQL 存储和缓冲的基本单位，默认 8KB | heap、B-Tree、FSM["free space map"]、VM、WAL replay、buffer manager 都围绕 page/block 运行 |
...
| free space map / FSM["free space map"] | relation 的空闲空间摘要 | INSERT 找可用 page、VACUUM 暴露可复用空间会用到 FSM["free space map"] |
...
| VACUUM | 清理 dead tuple、维护 VM/FSM["free space map"]、freeze XID、清理索引的后台路径 | PostgreSQL 长期稳定的核心机制，不是可选维护动作 |
...
- VACUUM 把旧版本、index entry、line pointer、VM、FSM["free space map"] 和 XID horizon 一起往前推。
...
| 高更新业务表 | HOT、VACUUM、FSM["free space map"]、VM、autovacuum | bloat、index cleanup、freeze 和 fillfactor 影响长期稳定 |
...
flowchart TB
    ...
    subgraph Heap["heap - table data authority"]
        ...
        Header["tuple header (xmin/xmax/ctid)"]
        VM["visibility map"]
        FSM["free space map"]
        Toast[TOAST table]
    end
...
```mermaid
flowchart LR
    Rel[relation] --> Main["main fork: heap/index pages"]
    Rel --> FSM["free space map fork"]
    Rel --> VM["visibility map fork (all-visible/all-frozen)"]
    Rel --> Init["init fork (unlogged reset)"]
...
普通表的 main fork 里是 heap page。FSM["free space map"] 和 VM 是两个容易被忽略但很重要的辅助状态：

- FSM["free space map"] 不是主数据，它帮助 INSERT 找到可能有空闲空间的 page。
- VM 不是权限或事务表，它是 page 级可见性摘要，影响 index-only scan 和 VACUUM 跳页。
- main fork、FSM["free space map"]、VM 的推进节奏不同。数据可见、空间可复用、页面全可见，不是同一件事。
...
```mermaid
flowchart TB
    Page[heap page] --> Header["page header (LSN/flags/lower/upper)"]
    Page --> LP[line pointer array]
    Page --> Free[free space]
    Page --> Items[tuple items]

    LP --> LP1["LP_NORMAL: points to tuple"]
    LP --> LP2["LP_REDIRECT: HOT redirect"]
    LP --> LP3["LP_DEAD: index may still point here"]
    LP --> LP4["LP_UNUSED: slot reusable"]

    Items --> Tup[heap tuple]
    Tup --> TH["tuple header (xmin/xmax/ctid/infomask)"]
...
```mermaid
flowchart TB
    Old["old heap tuple (xmin committed)"] --> Update[UPDATE arrives]
    Update --> Check[visibility and lock check]
    Check --> New["new heap tuple (xmin=updater XID)"]
    ...
    IndexedCols -->|no| HOT["HOT update (new tuple heap-only)"]
...
DELETE 比 UPDATE 简单，但和 UPDATE 有本质区别。DELETE 不会产生新 tuple version，它只做一件事：把目标 tuple 的 `xmax` 设置为执行 DELETE 的事务 XID。

```mermaid
flowchart TB
    Target["target heap tuple (xmin committed, visible)"] --> Check[visibility and lock check]
    Check --> Mark[set tuple xmax = deleting XID]
    Mark --> WALDel[write heap WAL record for delete]
    Mark --> Prune{page pruning or VACUUM later}
    Prune --> LP_DEAD[mark line pointer LP_DEAD]
    LP_DEAD --> IndexCleanup[VACUUM removes index entries]
    IndexCleanup --> LP_UNUSED[line pointer becomes LP_UNUSED]
    LP_UNUSED --> FSM["free space info goes into FSM"]
```

关键差异：

- UPDATE 写入新 tuple + 旧 tuple `xmax` 标记，DELETE 只有 `xmax` 标记，不写入新 tuple。
- DELETE 不会触发 HOT 判断——没有新 tuple，谈不上"新版本能否放入同页"。
- DELETE 不会立即清理 index entries。B-Tree 中的 index tuple 仍然指向这个 heap TID，只有后续 VACUUM 或 page pruning 才能把 dead index tuple 移除。
- 在并发场景下，DELETE 和 UPDATE 的锁冲突行为相同：两个事务不能同时 DELETE 同一行。

源码锚点：`heap_delete()` 在 `src/backend/access/heap/heapam.c` 中，核心逻辑是设置 `xmax`、标记 infomask、写 WAL record，然后由 executor 跳过该版本的可见性判断。与 `heap_update()` 不同，它不调用 `ExecInsertIndexTuples()`，也不会产生新 tuple。

这是 PostgreSQL 最重要的一个不对称：INSERT 写新 tuple + index tuples，UPDATE 写新 tuple + 可能多 index tuples（非 HOT 时），DELETE 不写任何新东西但把清理工作全部推迟给 VACUUM。因此高 DELETE 负载的表，VACUUM 是不可绕过的后台推进路径。
...
logical decoding 从 WAL 中提取行级变更。消费侧获得的内容由 replica identity 决定：

| replica identity | old tuple 内容 | 典型用途 |
| --- | --- | --- |
| `DEFAULT`（默认） | 只有主键列（或 replica identity index 列）的旧值 | 轻量 CDC，按 key 定位变更行 |
| `FULL` | 整行旧值（old tuple 全部列） | 需要完整审计、需要对比新旧值的消费端 |
| `NOTHING` | 无旧值 | 只关心新数据，不需要旧值对比 |
| `USING INDEX <index>` | 指定唯一索引列的旧值 | 精确控制 old tuple 开销 |

消费侧还需关注几个边界：

- **change type**：logical decoding 输出 INSERT / UPDATE / DELETE 三类变更。UPDATE 输出 old key（至少 replica identity 列）+ new tuple（全列或仅变更列，取决于 publication 配置）。
- **大事务与 reorder buffer**：大事务的变更会在 reorder buffer 中累积，直到事务提交才输出。如果事务很大且 consumer 消费慢，reorder buffer 可能占用大量内存并 spill 到磁盘。`logical_decoding_work_mem` 决定内存上限。
- **TOAST 与大字段**：logical decoding 默认不解码 TOAST 字段内容，消费端拿到的是 TOAST 引用。如果消费者需要完整大字段内容，需要设置 `REPLICA IDENTITY FULL` 并且确保 TOAST 值仍在 WAL 可恢复范围内。
- **DDL 在 logical stream 中**：PostgreSQL 原生 logical replication 不同步 DDL（schema change），消费者需要单独处理。这是很多 CDC 方案的补充环节。

源码锚点：
- `src/backend/replication/logical/decode.c`：`DecodeInsert()`、`DecodeUpdate()`、`DecodeDelete()` 构建逻辑变更记录。
- `src/backend/replication/logical/reorderbuffer.c`：`ReorderBufferChange`、`ReorderBufferTXN` 管理事务内变更的重排与暂存。
...
PostgreSQL 的 XID 是 32-bit 整数，最多约 20 亿个事务后就会回卷（wraparound）。这不是理论边界——如果 XID 回卷没有被 freeze 机制及时阻断，新事务的 XID 会看起来"比"旧 tuple 的 XID 更老，导致 MVCC 可见性判断彻底失效，数据"凭空消失"。

freeze 是唯一的防线：将足够老的 tuple 的 XID 替换为 `FrozenTransactionId`（值为 2），表示"这个 tuple 对所有事务永远可见"，不再需要与当前事务 XID 比较。freeze 由 autovacuum 在满足 `autovacuum_freeze_max_age` 条件时触发。如果 autovacuum 关闭或跟不上写入速度，数据库会在接近回卷边界时进入紧急模式，强制阻止新事务直到 freeze 完成。

这是 PostgreSQL 特有的、不可绕过的存储安全边界。后续讨论 Aurora/Neon 的 page server、TiDB 的 GC safepoint、Lakehouse 的 snapshot retention 时，都会回到同一类问题："旧版本的保留上限在哪里，越过后会发生什么"。
...
flowchart LR
    ...
    WAL --> PhysicalStandby["physical standby (redo page changes)"]
    WAL --> LogicalDecode["logical decoding (decode row changes)"]
...
stateDiagram
    ...
    LP_UNUSED --> FSM["free space map"]: free space becomes reusable
...
autovacuum 不是"按固定时间跑一次 VACUUM"，而是基于以下触发条件：

- **dead tuple 触发**：当表上 INSERT/UPDATE/DELETE 产生的 dead tuple 数量超过 `autovacuum_vacuum_threshold + autovacuum_vacuum_scale_factor * reltuples` 时触发 VACUUM。默认 threshold 50 + scale factor 0.2，意味着小表大约 50 行 dead tuple、大表约 20% 的行变成 dead 后触发。
- **freeze 触发**：当表的 `age(relfrozenxid)` 超过 `autovacuum_freeze_max_age`（默认 2 亿）时触发 freeze 型 VACUUM，目的是推进 freeze horizon，防止 XID wraparound。
- **analyze 触发**：类似 dead tuple 触发条件，但针对统计信息更新（`autovacuum_analyze_threshold` / `autovacuum_analyze_scale_factor`）。

autovacuum 由 `autovacuum launcher` 进程定期轮询决定哪个数据库/表需要启动 worker。worker 数量受 `autovacuum_max_workers` 限制（默认 3）。在高更新 OLTP 中，如果 worker 不够或 I/O 被限流（`autovacuum_vacuum_cost_limit`），就会出现 VACUUM 跟不上写入速度的情况，表现为 dead tuple 持续增长、表膨胀、索引膨胀、freeze 不及时。

### 后台写进程

三个后台写进程各司其职，决定 dirty page 和 WAL 何时落盘：

| 进程 | 职责 | 对性能的影响 |
| --- | --- | --- |
| **bgwriter**（background writer） | 定期扫描 shared buffers，把 dirty pages 提前写回数据文件，减轻 checkpoint 时的 I/O 尖峰 | 如果太慢（`bgwriter_lru_maxpages` 太小），checkpoint 需要写大量 dirty pages；太快则浪费 I/O |
| **walwriter**（WAL writer） | 定期把 WAL buffer 中的记录 flush 到磁盘，减少事务提交时等待 WAL flush 的延迟 | 如果 `wal_writer_delay` 太长，提交延迟增加；太短则产生更多小 I/O |
| **checkpointer** | 定期创建 checkpoint：扫描所有 dirty buffers 并写回，更新控制文件中的 redo point | 两个关键参数：`checkpoint_timeout`（默认 5 min）和 `max_wal_size`（WAL 增长阈值）。checkpoint 太频繁导致全量 I/O 抖动（full-page write 放大），太长则崩溃恢复时间增加 |

这三个进程合并决定了 PostgreSQL "数据落盘"的实际节奏：commit 成功不等于 page 落盘，page 落盘不等于 checkpoint 完成，checkpoint 完成不等于旧 WAL 可回收。
...
```mermaid
flowchart TB
    Ext[extension] --> SQLObj["SQL objects (types/funcs/operators)"]
    Ext --> IndexAM["index access method (opclass/opfamily)"]
...
heap 是主数据 authority，B-Tree 是访问路径，VM/FSM["free space map"] 是摘要状态，WAL 是恢复材料，catalog 是元数据状态，VACUUM 是长期推进器。任何一个状态落后，都会表现为性能、空间或可用性问题。
...
没有 VACUUM，heap tuple、index tuple、VM、FSM["free space map"]、freeze horizon、catalog cleanup 都不能健康推进。PostgreSQL 的长期稳定性，很大程度取决于后台回收路径是否能跟上前台写入和更新。
