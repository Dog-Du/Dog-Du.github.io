...
- 当前学习总天数：`14`
- 当前最近一次学习主题：`扩展：把 LazyVim 手感迁移到 Zed`
- 当前主线阶段：`14 天主线已完成；当前进入项目驱动扩展与复训`
- 当前系列：`Vim 14 天`
- 系列整理状态：
...
- 上一篇文章写到：
  ...
  - 扩展文章 `把 LazyVim 手感迁移到 Zed` 已创建，重点是把 Day 014 的编辑选择器迁移到 Zed 的 Vim mode、leader 风格键位、项目搜索、代码动作、diagnostics、Git 与终端闭环中
- 已学过主题：
  - `Day 000：总览与环境准备`
  - `Day 001：编辑心智模型与模式`
...
- 下一步建议：`不再新增 Day 015；直接进入项目驱动复训，并按薄弱点回看 Day 002 / 005 / 008 / 009 / 011 / 012 / 013 / 014；Zed 相关内容作为扩展文章维护`
...
| 日期 | 主题 | 文章文件 | 状态 |
| --- | --- | --- | --- |
| 2026-05-25 | `把 LazyVim 手感迁移到 Zed` | `learning-vim-extension-2026-05-25-zed-vim-lazyvim-workflow.md` | `done` |

说明：

- 扩展文章不计入 `14` 天主线编号。
- Zed 相关内容按真实工作流和具体问题补充，不再机械新增 Day 015。
