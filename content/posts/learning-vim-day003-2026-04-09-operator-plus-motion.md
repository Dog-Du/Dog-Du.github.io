...
5. 在第四行复制从光标处到行尾的内容，使用 `y$`。
6. 单独试一次：
   - `dd`
   - `yy`
   - `cc`
7. 每做完一步都按 `Esc` 回到普通模式，观察自己是不是在“先决定操作，再给范围”。
...
- `x`
  - 可以理解成一个非常短范围的删除，但今天主线优先建立组合思维。
- `D`
  - 常可理解成删到行尾，和 `d$` 在很多场景里接近。
- `C`
  - 常可理解成改到行尾，和 `c$` 在很多场景里接近。
- `Y`
  - 历史语义上容易和 `y$`、`yy` 混着记，今天先不拿它做主线。
