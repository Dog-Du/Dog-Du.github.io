...
6. 把光标放在第三行，用 `e` 和 `ge` 感受“词尾”定位。
7. 如果你的窗口足够窄，制造软换行后分别试 `j` / `k` 和 `gj` / `gk` 的差异。
...
- `W` / `B` / `E`
  - 按更粗粒度的 `WORD` 移动，后续可作为补充理解。
- `g_`
  - 到行尾最后一个非空白字符，适合某些代码行尾定位。
- `f{char}` / `t{char}`
  - 行内按字符快速定位，后面可以作为高频增强。
- `{count}w`、`3j`、`5G`
  - count 能自然叠加到大部分 motion 上，但今天先不要求大量使用。
